...
namespace DMTCommands
{
    class Reinforcer
    {
        ...
        public Reinforcer(_CONNECTION c)
        {
            _c = c;
        }
        ...
        public void run()
        {
            ...
            if (settings == false) throw new DMTException("[ERROR] Settings block");

            List<G.Line> polys = Reinforcer_Inputs.getSelectedPolyLines();
            if (!(polys.Count > 2)) throw new DMTException("[ERROR] Geometry");

            G.Point insertPoint = Reinforcer_Inputs.getBendingInsertionPoint();
            if (insertPoint == null) throw new DMTException("[ERROR] Bending insertion point");
